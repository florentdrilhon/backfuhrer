...
class BeerForm(FlaskForm):
    ...
    description = StringField('Courte description de la bière:', validators=[DataRequired()])
    detailed_description = TextAreaField('Description détaillée de la bière:', validators=[DataRequired()])
...
def beers():
    ...
    if request.method == 'POST' and form.is_submitted():
        try:
            ...
            beer = Beer(name=name, description=description, detailed_description=detailed_description,
                        price=price,
            ...
            logger.warning(f'Error when inserting the beer'
                           ...
                           f'Error: {err}')
            block_title = 'Ooops'
            message = f'Ouula oops, on a rencontré une petite erreur en insérant ta bière en base de données 😅'
