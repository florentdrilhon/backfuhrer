...
def test_list_by_number_player():
    ...
    not_good_games2 = list_of(lambda: utils.new_game(number_of_players=(random_number(8,10), random_number(15,20))))
    retrieved_games = games_repository.list_by(number_players=7)
    for game in good_games:
        assert game in retrieved_games
    for game in not_good_games + not_good_games2:
        assert game not in retrieved_games
    for game in retrieved_games:
        assert game.number_of_players[0] <= 7
        assert game.number_of_players[1] >= 7
